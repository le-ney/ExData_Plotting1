...
#(./path../filename). The output of this function is plot_3.png file containing
#line chart for energy sub metering and Date & tiem. Please note the generated PNG file 
#can be found in your R current work directory
plot.3 <- function(fileName){
        ...
        dataSet <- dataSet[,c(1,7:9)] #select only the required fields
        dataSet <- dataSet[(as.Date(dataSet$Date) >= as.Date("2007-02-01")) & 
                                   (as.Date(dataSet$Date) <= as.Date("2007-02-02")),] #filter for certain dates
        dataSet [,2:4] <- lapply(dataSet [,2:4],as.numeric) #convert all fields except Date to numeric
        png(file="plot_3.png", height=480, width=480) #set and open PNG file
        with(dataSet, {
                plot(Date,Sub_metering_1, xlab="", ylab="Energy sub metering",pch=NA) 
                lines(Date, Sub_metering_1)
                lines(Date, Sub_metering_2, col='red')
                lines(Date, Sub_metering_3, col='blue') #plot line chart
        })
        #set legend
